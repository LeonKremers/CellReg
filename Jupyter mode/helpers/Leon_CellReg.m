%% script to run the entire CellReg pipeline headlessly, by being called from a Jupyter notebook
%% all CellReg settings are defined in the jupyter notebook and saved in a settings.mat file in the data folder, which is loaded here.
%% Uses Suite2P output files and converts them to the required input format for CellReg
%% Adapted from the original CellReg code by Sheintuch et al., 2017
%% Author: Leon Kremers, 2026


%% Add required file path
addpath ("/home/leonk/CellReg")
addpath ("/home/leonk/CellReg/Helper")
addpath ("/home/leonk/CellReg/CellReg")
addpath ("/home/leonk/CellReg/GUI")

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Specifiy data path, file names and settings
%load path from temp_folder.mat file
data = load('/home/leonk/adamacs/notebooks/temp/temp_folder.mat');
files_path = data.folder_path;

%load settings from settings.mat file in files_path
settings = load(append(files_path,'/settings.mat'));

% Specify the file extension/type you want to extract
fileExtension = '*.mat'; 
% Create the full file path pattern
filePattern = fullfile(files_path, fileExtension);
% Use the dir function to list all files with the specified extension
files = dir(filePattern);
%skip already converted files and settings file
exclude_string = 'converted';
exclude_string2 = 'settings';
file_names = {files.name};
files = files(cellfun(@(x) isempty(strfind(x, exclude_string)) && isempty(strfind(x, exclude_string2)), file_names));
...
%main code wrapped into try function to catch errors and create error log file
try
    ...
    input_format=settings.input_format; % could be either 'CNMF-e' or 'Suite2p'

    % Create a log file with progress
    log = fopen(fullfile(results_directory, 'log.txt'), 'wb');
    fprintf(log, '%s\n', "Performing Session Conversion");

    %% Choosing the files for conversion:   
    for k = 1:length(files)
        disp("Now converting:")
        files(k).name
        file_names = files(k).name;

        if exist(append(files_path, "converted_", files(k).name), 'file') == 2
            disp("Converted file already exists. Skipping")
            disp("==================")
        else
            disp("==================")
            ...
            for n=1:number_of_sessions
                ...
                if iscell(file_names)
                    save([files_path 'converted_' file_names{n}],'this_session_converted_footprints','-v7')
                else
    ...
    memory_efficient_run = settings.memory_efficient_run;
    ...
    % define path of data
    number_of_sessions=length(files)
    file_names=cell(1,number_of_sessions);
    for it = 1:number_of_sessions
        file_name = num2str(files(it).name);
        file_name = ['converted_', file_name];
        file_names{it} = fullfile(fileroot, file_name);
    end

    if memory_efficient_run
        temp_dir = append(figures_directory, filesep, 'temp'); 
        if ~exist(temp_dir)
            mkdir(temp_dir);
        end
    end
    ...
    alignment_type=settings.alignment_type; % either 'Translations', 'Translations and Rotations' or 'Non-rigid'
    use_parallel_processing=settings.use_parallel_processing; % either true or false
    maximal_rotation=settings.maximal_rotation; % in degrees - only relevant if 'Translations and Rotations' is used
    transformation_smoothness=settings.transformation_smoothness; % levels of non-rigid FOV transformation smoothness (range 0.5-3)
    reference_session_index=settings.reference_session_index; 
    ...
    sufficient_correlation_centroids=settings.sufficient_correlation_centroids; % smaller correlation imply no similarity between sessions
    sufficient_correlation_footprints=settings.sufficient_correlation_footprints; % smaller correlation imply no similarity between sessions
    ...
    maximal_distance=settings.maximal_distance; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells
    normalized_maximal_distance=maximal_distance/microns_per_pixel;
    p_same_certainty_threshold=settings.p_same_certainty_threshold; % certain cells are those with p_same>threshld or <1-threshold

    % Computing correlations and distances across days:
    disp('Stage 3 - Calculating a probabilistic model of the data')
    fprintf(log, '%s\n', 'Stage 3a - Computing data distribution');
    ...
    fprintf(log, '%s\n', 'Finished computing data distribution');
    ...
    fprintf(log, '%s\n', 'Stage 3b - Calculating a probabilistic model of the data');
    ...
    disp('Done')
    fprintf(log, '%s\n', 'Finished calculating a probabilistic model of the data');
    ...
    initial_registration_type = settings.initial_registration_type
    if strcmp(initial_registration_type, 'best_model_string') % either 'Spatial correlation', 'Centroid distance', or 'best_model_string';
    ...
    disp('Done')
    fprintf(log, '%s\n', 'Finished initial registration');
    fprintf(log, '%s\n', [num2str(size(cell_to_index_map,1)) ' cells were found']);
    ...
    registration_approach=settings.registration_approach; % either 'Probabilistic' or 'Simple threshold'
    model_type = settings.model_type
    if strcmp(model_type, 'best_model_string') % either 'Spatial correlation' or 'Centroid distance'
        model_type=best_model_string;
    end
    p_same_threshold=settings.p_same_threshold; % only relevant if probabilistic approach is used
...
%Create a log file with the error message
    ...
    % Create error message in log file
    log = fopen(fullfile(results_directory, 'log.txt'), 'wb');
    fprintf(log, '%s\n', "Error occured:");
    fprintf(log, '%s\n', ME.message);
    fclose(log);
